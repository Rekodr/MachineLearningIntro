...
int main(int argc, char* argv[]) {
    ...
    DataReader reader1 = DataReader("handWrittenDigitNormalized-training.data", make_pair(10, 64), 10);
    DataReader reader2 = DataReader("handWrittenDigitNormalized-testing.data", make_pair(1797, 64), 10);

    vector<unsigned> networkStruct{64, 42, 10};
    auto X = reader1.getX();
    auto Y = reader1.getY();
    NeuralNet neuralNet = NeuralNet(X, Y, networkStruct, 0.1, 200);
    neuralNet.train();
