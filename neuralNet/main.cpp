...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    DataReader reader1 = DataReader("handWrittenDigitNormalized-training.data", make_pair(3823, 64), 10);
    DataReader reader2 = DataReader("handWrittenDigitNormalized-testing.data", make_pair(1797, 64), 10);

    vector<unsigned> networkStruct{64, 24, 10};
    auto X = reader1.getX();
    auto Y = reader1.getY();
    NeuralNet neuralNet = NeuralNet(X, Y, networkStruct, 0.3, 50);
    neuralNet.train();

    auto X_test = reader2.getX();
    auto Y_test = reader2.getY();
